...
//! on MCU chips. It includes the `PeripheralRef` struct, which represents an
        ...
        ($($(#[$cfg:meta])? $name:ident),*$(,)?) => {
            ...
            impl Peripherals {
                ...
                pub fn take() -> Self {
                    ...
                    #[no_mangle]
                    static mut __HAL_DEVICE_PERIPHERALS: bool = false;

                    critical_section::with(|_| unsafe {
                        if __HAL_DEVICE_PERIPHERALS {
                            panic!("init called more than once!")
                        }
                        __HAL_DEVICE_PERIPHERALS = true;
                        Self::steal()
                    })
                }
            }
